 ...
 * 
 */
public class GgssActivity extends BaseActivity implements OnClickListener {
	private GGSS attrs;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_ggss);
		initData();
		initView();
	}
	...
	private void initData() {
		...
		attrs = (GGSS) getIntent().getSerializableExtra("attrs");
		if (attrs.ID == null) {
			isEdit = false;
		}
	}

	private void fillBean(String name, String zname, String lxr, String lxdh,
			String bz) {
		attrs.MC = name;
		attrs.ZMC = zname;
		attrs.BZ = bz;
				...
				DictDialogUtil.changeType(this, new MapDictCallback() {
					...
					public void call(MapLevelDictObject mapLevelDictObject) {
						TypeUtil.changeType(mapLevelDictObject, attrs,
								GgssActivity.this, WAIT_FOR_RESULT);
					}
				});
			}
			break;
		...
		case R.id.bt_save:
			if (TextUtils.isEmpty(input_name.getShowView().getText().toString()
				...
				boolean same = name.equals(attrs.MC) && zname.equals(attrs.ZMC)
						&& lxr.equals(attrs.LXR) && lxdh.equals(attrs.LXDH)
						&& bz.equals(attrs.BZ);
				...
				if (isEdit) {
					...
					if (!same) {
						attrs.fillXgxx();
						fillBean(name, zname, lxr, lxdh, bz);
