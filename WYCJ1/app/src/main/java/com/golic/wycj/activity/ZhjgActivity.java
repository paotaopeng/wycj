...
public class ZhjgActivity extends BaseActivity implements OnClickListener
{
	private ZHJG attrs;
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		setContentView(R.layout.activity_zhjg);
		initData();
		initView();
	}
	...
	private void initView()
	{
		...
		input_lxr = (InputView) findViewById(R.id.input_lxr);
		input_lxdh = (InputView) findViewById(R.id.input_lxdh);
		input_gj = (InputView) findViewById(R.id.input_gj);
		...
		input_lxr.getShowView().setText(attrs.LXR);
		input_lxdh.getShowView().setText(attrs.LXDH);
		input_gj.fillDictInputView(dict, attrs.GJ);
	...
	private void initData()
	{
		...
		attrs = (ZHJG) getIntent().getSerializableExtra("attrs");
		if (attrs.ID == null)
		{
			isEdit = false;
		}
	...
	private void fillBean(String name, String zname, String bz, String lxr,
			String lxdh, String gj)
	{
		attrs.MC = name;
		attrs.ZMC = zname;
		attrs.BZ = bz;
	...
	public void onClick(View v)
	{
				...
				DictDialogUtil.changeType(this, new MapDictCallback()
				{
					...
					public void call(MapLevelDictObject mapLevelDictObject)
					{
						TypeUtil.changeType(mapLevelDictObject, attrs,
								ZhjgActivity.this, WAIT_FOR_RESULT);
					}
				});
			}
			break;
		...
		case R.id.bt_save:
			...
			else
			{
				...
				String lxr = input_lxr.getShowView().getText().toString()
						.trim();
				String lxdh = input_lxdh.getShowView().getText().toString();
				String gj = "";
				DictObject gjDict = (DictObject) input_gj.getTag();
				if (gjDict != null)
				{
					gj = gjDict.getDM();
				}
				...
				if (isEdit)
				{
					...
					else
					{
						...
						fillBean(name, zname, bz, lxr, lxdh, gj);
