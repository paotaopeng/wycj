...
public class CsActivity extends BaseActivity implements OnClickListener
{
	...
	private void initData()
	{
		...
		attrs = (CS) getIntent().getSerializableExtra("attrs");
		if (attrs.ID == null)
		{
			isEdit = false;
		}
				...
				DictDialogUtil.changeType(this, new MapDictCallback()
				{
					...
					public void call(MapLevelDictObject mapLevelDictObject)
					{
						TypeUtil.changeType(mapLevelDictObject, attrs,
								CsActivity.this, WAIT_FOR_RESULT);
					}
				});
			}
			break;
		...
		case R.id.bt_save:
			...
			else
			{
				...
				if (isEdit)
				{
					...
					boolean same = name.equals(attrs.MC)
							&& zname.equals(attrs.ZMC) && bz.equals(attrs.BZ);
					// 判断地址数据是否发生修改
					boolean sameDz = mphm.MLXZ.equals(attrs.mphm.MLXZ)
							&& mphm.BZ.equals(attrs.mphm.BZ);
					if (!sameDz)
					{
					...
					else
					{
						...
						fillBean(name, zname, bz, lxr, lxdh);
	...
	// /**
	// * 跟新公共数据的地址，顺带的把公共数据表中的DZ字段一并跟新
	// *
	// * @param attrs
	// */
	// private void updateDz(BaseAttrs attrs)
	// {
	// mphm.ID = attrs.mphm.ID;
	// mphm.DJR = attrs.mphm.DJR;
	// mphm.JWH = attrs.mphm.JWH;
	...
	// mphm.XGR = attrs.XGR;
	// mphm.GXSJ = attrs.GXSJ;
	// int update = bzdzDaoImpl.updateGgsjDz(attrs, mphm, extraDz);
	// if (update > 0)
	// {
	// Source.updateGgsj(attrs);
	// }
